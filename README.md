...
| `sentimentosOncologia.csv` | Comentários de pacientes oncológicos (EN) com análise de sentimento |
| `sobrevivenciaCancer.csv` | Taxas de sobrevivência por tipo de tumor e estágio |
| `tempoP_inicioTratamento.csv` | Tempo até início do tratamento por região brasileira |
...
pip install -r requirements.txt
# Coloque todos os arquivos .csv na pasta
streamlit run main.py
```
...
- Python 3.10+
- Os arquivos `.csv` **não estão versionados** (`.gitignore`). 
